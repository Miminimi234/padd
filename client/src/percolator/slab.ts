/**
 * Percolator Padd Transaction Builders
...
/**
 * Initialize a new Padd (10MB account for orderbook)
 * 
 ...
 */
export async function initPadd(
  marketId: PublicKey,
  authority: PublicKey,
  risk: RiskParams,
  ...
  // Find Padd state PDA (program will create this account)
  const [PaddState] = findPaddStatePda(marketId);
  const [PaddAuthority] = findPaddAuthorityPda(PaddState);
  ...
  data.writeUInt8(DISCRIMINATORS.INIT_Padd, 0);
  ...
  const ix = new TransactionInstruction({
    programId: Padd_PROGRAM_ID,
    keys: [
      { pubkey: payer, isSigner: true, isWritable: true },
      { pubkey: marketId, isSigner: false, isWritable: false },
      { pubkey: authority, isSigner: false, isWritable: false },
      { pubkey: PaddState, isSigner: false, isWritable: true },
      { pubkey: PaddAuthority, isSigner: false, isWritable: false },
...
/**
 * Add an instrument (tradeable contract) to a Padd
 */
...
export async function reserve(
  ...
  const [PaddState] = findPaddStatePda(params.routeId);
...
export async function liquidate(
  ...
  const [PaddState] = findPaddStatePda(marketId);
  const [position] = findPositionPda(trader, PaddState, instrumentIndex);
