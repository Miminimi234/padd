...
export async function mintCap(
  ...
  // Find PDAs
  const [escrow] = findEscrowPda(params.user, params.Padd, params.mint);
  const [cap] = findCapPda(params.user, params.Padd, params.mint, nonce);
  ...
  const ix = new TransactionInstruction({
    ...
    keys: [
      ...
      { pubkey: params.Padd, isSigner: false, isWritable: false },
