...
// Raydium LaunchLab Service for Padd platform
export class RaydiumLaunchLabService {
    ...
    /**
     * Create Padd platform configuration (one-time setup)
     * This defines our branding, fee rates, etc.
     */
    async createPaddPlatformConfig(wallet: WalletContextState) {
        ...
        try {
            ...
            console.log('Creating Padd Platform Config...');

            const platformConfigParams = {
                owner: wallet.publicKey,
                feeRate: 1000,      // 1% platform fee for Padd
                creatorFeeRate: 2000, // 2% to project creator
                name: 'Padd Trading Platform',
                web: 'https://Padd.trade',
                img: 'https://Padd.trade/Paddlogo.png',
            };

            console.log('Padd Platform Config Params:', platformConfigParams);
            return platformConfigParams; // Return params for now until SDK is properly configured
        } catch (error) {
            console.error('Failed to create Padd platform config:', error);
            throw error;
        }
    }
    ...
    prepareLaunchParams(
        ...
        platformConfig: any, // Platform config from createPaddPlatformConfig
...
export interface PaddPlatformConfig {
    owner: PublicKey;
