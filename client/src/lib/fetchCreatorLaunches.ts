...
const DEFAULT_PLATFORM_ID = import.meta.env.VITE_Padd_PLATFORM_CONFIG_ID?.trim() ?? FALLBACK_PLATFORM_ID;
...
export async function fetchCreatorLaunches({
    ...
    for (const [, info] of entries) {
        ...
        const symbolFallback = mintAddress ? mintAddress.slice(0, 6).toUpperCase() : "Padd";
