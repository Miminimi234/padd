export type Theme = 'ocean' | 'forest' | 'graphite';
...
export const themes: Record<Theme, ThemeConfig> = {
  ...
  graphite: {
    ...
    colors: {
      ...
      muted: '0 0% 85%', // much lighter gray - almost white for better readability
      success: '142 76% 36%', // green-500
      warning: '38 92% 50%', // amber-500
      destructive: '0 84% 60%', // red-500
    }
...
export function applyTheme(theme: Theme) {
  ...
  if (!themeConfig || !themeConfig.colors) {
    console.warn(`Theme '${theme}' is not defined or missing colors. Falling back to 'graphite'.`);
    if (theme !== 'graphite') {
      return applyTheme('graphite');
    }
    return;
  }
  ...
  root.style.setProperty('--muted-foreground', themeConfig.colors.text); // Use main text color for muted text
  ...
  // Set data attribute for theme-specific CSS overrides
  root.setAttribute('data-theme', theme);

  // For graphite theme, ensure all text is either white or orange
  if (theme === 'graphite') {
    root.style.setProperty('--card-foreground', themeConfig.colors.text);
    root.style.setProperty('--popover-foreground', themeConfig.colors.text);
    root.style.setProperty('--sidebar-foreground', themeConfig.colors.text);
    root.style.setProperty('--sidebar-accent-foreground', themeConfig.colors.text);
  }

  // Save to localStorage
  localStorage.setItem('Padd-theme', theme);
}

export function getStoredTheme(): Theme {
  const stored = localStorage.getItem('Padd-theme');
  if (!stored) return 'graphite';

  // Validate stored value is a supported theme
  if (Object.prototype.hasOwnProperty.call(themes, stored)) {
    return stored as Theme;
  }

  console.warn(`Stored theme value '${stored}' is not a valid theme. Falling back to 'graphite'.`);
  return 'graphite';
}

export function initializeTheme() {
  const theme = getStoredTheme() || 'graphite';
  applyTheme(theme);
  return theme;
}
