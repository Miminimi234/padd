...
export default function Market() {
  ...
  const routeSymbol = params?.symbol ?? "Padd";
  ...
  // Trade ticket state
  const [tradeMode, setTradeMode] = useState<"Padd" | "spot">("Padd");
        ...
        <Card className="border-primary/20 bg-card p-8 text-center">
          ...
          <div className="text-xs text-muted-foreground">
            {marketError === "MARKET_NOT_FOUND"
              ? "Requested market not found in Padd registry or Jupiter feed."
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 items-stretch">
                ...
                <TabsContent value="orderbook" className="h-full p-2">
                  <div className="h-full overflow-y-auto">
                    <div className="text-center py-8 text-muted-foreground text-xs">
                      No Padd market active on this token
                    </div>
                  </div>
                </TabsContent>
              </Tabs>
        ...
        <div className="xl:col-span-3">
          <Card className="border-primary/20 bg-card h-full flex flex-col">
            ...
            <Tabs defaultValue="Padd" className="flex-1">
              <div className="border-b border-primary/20">
                <TabsList className="bg-transparent border-0 w-full grid grid-cols-2">
                  <TabsTrigger
                    value="Padd"
              ...
              <div className="flex-1 flex flex-col">
                ...
                >
                  {/* Padd trade UI (leverage) */}
                  <div className="px-4 pt-3">
                    <label className="text-[10px] font-semibold text-muted-foreground block mb-2">Select Padd</label>
                    <select
                      className="w-full rounded-md border border-primary/20 bg-card px-3 py-2 text-sm text-foreground"
                      defaultValue=""
                      aria-label="Select Padd"
                    >
                  ...
                  {/* Message & launch action when no Padds exist */}
                  <div className="px-4 text-center text-xs text-muted-foreground">
                    <div className="py-4">0 Padds exist for this token. Launch its first Padd.</div>
                    <div>
      ...
      <Card className="border-primary/20 bg-card">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
          ...
          <TabsContent value="positions" className="p-6">
            <div className="text-center py-8 text-muted-foreground text-xs">
              nothing here yet, open a Padd trade first
            </div>
          </TabsContent>
