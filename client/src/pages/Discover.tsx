...
type PaddSortOption = "newest" | "top";
...
function FirebaseTokenCard({ token }: FirebaseTokenCardProps) {
  ...
  const launchpadLabel = coerceString(tokenRecord.launchpad) ?? coerceString(tokenRecord.metaLaunchpad) ?? "Padd";
...
export default function Discover() {
  ...
  const [PaddSort, setPaddSort] = useState<PaddSortOption>("newest");
  ...
  const filteredFirebaseTokens = useMemo(() => {
    ...
    return filtered;
  }, [creatorQuery, PaddSort, firebaseCatalog]);
  ...
  return (
    <div className="space-y-6 px-4 py-4">
      ...
      >
        <div className="space-y-1">
          <h2 className="text-xl font-semibold">Padd Creator Tokens</h2>
          <p className="text-sm text-muted-foreground">
            Tokens made by creators across the Padd ecosystem.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button
              type="button"
              variant={PaddSort === "top" ? "default" : "outline"}
              onClick={() => setPaddSort("top")}
            >
              Sort by top Padd tokens
            </Button>
            <Button
              type="button"
              variant={PaddSort === "newest" ? "default" : "outline"}
              onClick={() => setPaddSort("newest")}
            >
              Sort by newest Padd tokens
            </Button>
          </div>
        </div>
